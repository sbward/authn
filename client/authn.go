...
func NewClient(config Config, app *authn.App) (*Client, error) {
	var err error
	config.setDefaults()
	...
	ac.kchain = &appJWKProviderAdapter{ac.app}
...
func Configure(config Config, app *authn.App) error {
	client, err := NewClient(config, app)
	if err != nil {
		return err
	}
